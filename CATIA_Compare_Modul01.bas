...
Private Const M01_TITLE As String = "CATIA Compare - Modul 01 | v1.1.5"
...
Public Sub M01_Baslat()
    ...
    LogLine "Gerekirse aktif Shape bellekte UpdateObject ile guncellenir; kaynak kaydedilmez."
...
Private Function OpenOrReuse(ByVal fullPath As String) As Object
    ...
    If Len(foundPath) = 0 Then
        Err.Raise vbObjectError + 2102, M01_TITLE, _
                  "Acilan belgenin yolu okunamadi."
    End If
...
Private Sub CheckModelDocument(ByVal modelDoc As Object, ByVal fullPath As String)
    If modelDoc Is Nothing Then
        Err.Raise vbObjectError + 2104, M01_TITLE, _
                  "Belge acilamadi: " & fullPath
    End If
...
Private Sub NoteSavedState(ByVal modelDoc As Object)
    ...
    If CBool(modelDoc.Saved) Then Exit Sub
...
Private Sub ScanNode(ByVal occurrence As Object, ByVal treePath As String, _
    ...
    If depth > M01_MAX_DEPTH Or stats.Nodes >= M01_MAX_NODES Then
        AddIssue stats, treePath, 0, _
                 "Tarama derinlik/dugum sinirina ulasti; sonuc eksik.", True
        Exit Sub
    End If
...
Private Sub ScanPart(ByVal partDoc As Object, ByVal treePath As String, _
    ...
    LogLine "BAGLAM v1.1.5: CATPart penceresi / " & ObjectName(partDoc)
...
Private Function M01_HasBody(ByVal bodies As Collection, _
                             ByVal body As Object) As Boolean
...
Private Sub InspectBody(ByVal part As Object, ByVal spa As Object, _
                        ...
                        ByRef stats As M01_Stats)
    Dim shapes As Object, feature As Object
    Dim reference As Object, measurable As Object
    Dim volumeM3 As Double
    Dim i As Long, count As Long, activeCount As Long
    Dim check As String, errorNumber As Long, errorText As String
    ...
    If shapes Is Nothing Then
        Err.Raise 91, M01_TITLE, "Shapes alinamadi."
    End If
    ...
    For i = 1 To count
        ...
        If feature Is Nothing Then
            Err.Raise 91, M01_TITLE, "Shape alinamadi."
        End If

        check = "B05." & CStr(i)

        If CBool(part.IsInactive(feature)) Then
            LogLine "ATLANDI (pasif Shape): " & treePath & "/" & ObjectName(feature)
        Else
            activeCount = activeCount + 1
            check = "B06." & CStr(i)

            currentState = part.IsUpToDate(feature)

            If IsNull(currentState) Or IsEmpty(currentState) Then
                Err.Raise vbObjectError + 2128, M01_TITLE, _
                          "Guncellik sonucu bos."
            End If

            LogLine "GUNCELLIK: " & treePath & "/" & ObjectName(feature) & _
                    " | " & TypeName(currentState) & "=" & CStr(currentState) & _
                    " | sayisal=" & CStr(CLng(currentState))

            If CBool(currentState) Then
                LogLine "GUNCEL OK: " & treePath & "/" & ObjectName(feature)
            Else
                check = "B06U." & CStr(i)

                LogLine "UPDATE DENEMESI (bellekte): " & treePath & _
                        "/" & ObjectName(feature)
                ...
                If CBool(currentState) Then
                    LogLine "UPDATE OK (kaydedilmedi): " & treePath & _
                            "/" & ObjectName(feature)
                Else
                    Err.Raise vbObjectError + 2107, M01_TITLE, _
                              "UpdateObject sonrasi halen guncel degil: " & _
                              ObjectName(feature)
                End If
            End If
        End If
    ...
    If spa Is Nothing Then
        Err.Raise 91, M01_TITLE, "Olcum arayuzu alinamadi."
    End If

    Set reference = part.CreateReferenceFromObject(body)

    If reference Is Nothing Then
        Err.Raise 91, M01_TITLE, "Body referansi alinamadi."
    End If
    ...
    If measurable Is Nothing Then
        Err.Raise 91, M01_TITLE, "Measurable alinamadi."
    End If
    ...
    If volumeM3 <= 0# Then
        Err.Raise vbObjectError + 2109, M01_TITLE, _
                  "Pozitif kati hacim yok."
    End If
...
Unverified:
    ...
    AddIssue stats, "IB115 / " & check & " / " & treePath, _
             errorNumber, errorText, True
End Sub


Private Function ApplyPreviewStyle(ByVal resultDoc As Object, _
                                    ByVal groupA As Object, _
                                    ByVal groupB As Object) As Boolean
...
Failed:
    ...
    If Len(mFirstIssue) = 0 Then
        mFirstIssue = "Yeni montaja renk uygulanamadi."
    End If
...
Private Sub SetResultVisibility(ByVal stateA As Long, ByVal stateB As Long)
    ...
    If Left$(root.PartNumber, Len(M01_PREFIX)) <> M01_PREFIX Then
        GoTo WrongDocument
    End If
...
Private Sub AddIssue(ByRef stats As M01_Stats, ByVal location As String, _
    ...
    If number = 91 Then
        MsgBox location, vbExclamation, "91 - Hata yeri"
    End If
...
Private Function SummaryWithPath() As String
    ...
    If Len(mFirstIssue) > 0 Then
        message = message & vbCrLf & vbCrLf & mFirstIssue
    End If

    If Len(mReportPath) > 0 Then
        message = message & vbCrLf & vbCrLf & _
                  "Ayrintili rapor:" & vbCrLf & mReportPath
    ElseIf Not mLog Is Nothing Then
        message = message & vbCrLf & vbCrLf & _
                  "Ayrintilar: VBA Immediate Window (Ctrl+G)."
    End If
...
Private Sub FinishReport()
    ...
    If Len(folder) = 0 Then
        Err.Raise vbObjectError + 2110, M01_TITLE, _
                  "Gecici rapor klasoru bulunamadi."
    End If
...
Failed:
    ...
    mLastSummary = mLastSummary & vbCrLf & _
                   "Rapor dosyasi yazilamadi: " & errorText
End Sub
...
Public Sub M01_Tani()
    Dim doc As Object, part As Object, body As Object
    Dim firstFeature As Object, lastFeature As Object
    Dim report As String, stage As String
    ...
    report = "M01 TANI - CATPart penceresi" & vbCrLf & _
             ...
             "Hedef: " & Left$(CStr(firstFeature.Name), 50) & vbCrLf & vbCrLf
    ...
    report = report & "5 SON SHAPE (" & _
             ...
             M01_TaniHacim(doc, part, lastFeature) & vbCrLf & vbCrLf
...
Failed:
    MsgBox report & vbCrLf & stage & " | " & CStr(Err.Number) & _
...
Private Function M01_TaniDurum(ByVal part As Object, _
    ...
    Else
        M01_TaniDurum = TypeName(raw) & "=" & CStr(raw) & _
                       "; Boolean=" & CStr(CBool(raw)) & _
                       "; sayisal=" & CStr(CLng(raw))
    End If
...
Private Function M01_TaniHacim(ByVal doc As Object, ByVal part As Object, _
                              ByVal obj As Object) As String
    Dim spa As Object, reference As Object, measurable As Object
    Dim volumeM3 As Double, stage As String
